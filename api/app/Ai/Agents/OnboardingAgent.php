...
use App\Ai\Tools\BuildOnboardingPlan;
 ...
 *
 *   Step 1 (always)  build_onboarding_plan       deterministic draft
 *   Step 2 (optional) get_recent_runs            inspect activity history
 *   Step 3 (optional) adjust_onboarding_plan     personalise based on notes
 ...
 * `adjust_onboarding_plan` (replace / add / remove / adjust ops, server
...
class OnboardingAgent implements Agent, Conversational, HasTools
{
    ...
    public function instructions(): string
    {
        ...
        ## Your job
        Step 1 — ALWAYS call `build_onboarding_plan` ONCE first, passing the form fields from the priming message verbatim. Pass null when a field is null. This produces the deterministic draft proposal.

        Step 2 — Read `additional_notes`:

          • Injury / pain / recovery (knee, achilles, ITB, patella, shin, hip, plantar, "coming back from", tendonitis): MUST call `adjust_onboarding_plan` to replace tempos AND intervals with easy in the first 4-8 weeks (use the runner's stated window if given). Do this even when the day count already matches their request — injury needs SESSION-TYPE changes, not just day count.
          • Training preferences ("more intervals", "no long runs Sundays", "extra day on Wed"): call `adjust_onboarding_plan` to swap/add/move sessions accordingly.
          • Empty / generic ("thanks!", "looking forward to it"): SKIP step 3.

        Step 3 (optional) — `adjust_onboarding_plan` operations are JSON. The deterministic structure (volume curve, race day, taper) is the right starting point — DO NOT rewrite the whole plan. Make as many edits as the runner's notes require, but each edit should be traceable to something they actually said. Server enforces guard rails: pace clamps (±15s tempo, ±10s interval work), distance clamps (4 km min, 1.5× builder max), no race-day touches, no easy-pace overrides.
        ...
        - DO NOT call `build_onboarding_plan` more than once.
        ...
        - If `build_onboarding_plan` returned an error, apologise in one sentence and suggest the runner widen their preferred weekdays or pick a later goal date.
        PROMPT;
    }
