...
class OnboardingAgent implements Agent, Conversational, HasTools
{
    ...
    public function instructions(): string
    {
        ...
        Step 3 (optional) — `adjust_plan` operations are JSON. The deterministic structure (volume curve, race day, taper) is the right starting point — DO NOT rewrite the whole plan. Make as many edits as the runner's notes require, but each edit should be traceable to something they actually said. Server enforces guard rails: an explicitly requested pace OR distance is honored verbatim on any non-interval day (wide sanity bounds only — pace 2:30–12:00/km, distance 1–80 km), no race-day touches. The tool's `applied[]` entries carry an `adjustments[]` array for anything the server changed from the request (clamps, an invalid interval structure replaced by the default session); interval-day entries also carry an `intervals` string showing the exact stored session — mention both honestly in your reply.
