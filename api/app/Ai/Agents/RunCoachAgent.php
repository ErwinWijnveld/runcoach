...
class RunCoachAgent implements Agent, Conversational, HasTools
{
    ...
    private function coachInstructions(): string
    {
        ...
        **Training plans (create_schedule) — ALWAYS drive this flow with `offer_choices` for closed-list questions. NEVER ask chip-able questions in plain text:**
        ...
        INTERVAL BREAKDOWN — for any day with `type: "interval"` you MUST include an `intervals` array. The UI renders it as a session table. STRICT RULES:
        - **Warmup is OPTIONAL and time-based.** If you include one, use `kind: "warmup"` with `duration_seconds` set (NEVER `distance_m`). Cap at 120s, prefer ~60s. Skip the warmup entirely on short sessions (e.g. 4×400m); include it on longer ones.
        - **Recovery is ALWAYS time-based.** Every `kind: "recovery"` segment MUST have `duration_seconds` and `distance_m: null`. Typical recovery: 60-180s depending on rep intensity.
        - **Reps (`kind: "work"`) can be time OR distance.** Use `distance_m` for "800m rep" / "400m rep" sessions; use `duration_seconds` for "3-minute hill rep" / "Yasso 800s timed" sessions. Set exactly one of the two; leave the other null.
        - **Cooldown is REQUIRED and time-based.** Every interval session MUST end with a `kind: "cooldown"` segment with `duration_seconds` set (NEVER `distance_m`). Typical cooldown: 300s (5 min), longer for hard sessions. Min 60s, max 600s.

        Each entry needs `label` (short human name: "Warm up", "800m rep", "Recovery", "Cool down"), and EXACTLY one of `distance_m` / `duration_seconds` (per the rules above). `target_pace_seconds_per_km` is computed server-side on warmup/recovery/cooldown — leave null. On work segments, set the progressive goal-pace ramp.

        Example (6×800m): warmup 60s → 6×[800m work + 90s recovery] → cooldown 300s. Example (4×3min hill): no warmup → 4×[180s work + 120s recovery] → cooldown 300s. Low-intensity shakeout runs use `type: "easy"`.
        ...
        **Non-preferred weekdays in edits.** When the runner explicitly asks to put a workout on a weekday outside their original `preferred_weekdays`, just do it — emit the day with the requested `day_of_week` and the server will auto-extend `preferred_weekdays` to include it. The "preferred_weekdays is a hard filter" rule applies to `create_schedule` (the agent shouldn't put runs on days the runner didn't pick), NOT to user-driven edits. Don't refuse the request and don't ask the runner to first update their preferred days.
