...
class CreateSchedule implements Tool
{
    ...
    public function description(): string
    {
        ...
        Use the SAME progression for `target_pace_seconds_per_km` on interval `work` segments. `warmup` / `recovery` / `cooldown` segments leave null — server defaults them. The progression is the whole point of quality work — do not set every tempo at the same pace.

        INTERVAL RULES (strict):
        - Warmup is optional, ALWAYS time-based (`duration_seconds` set, `distance_m: null`), max 120s, prefer ~60s.
        - Recovery between reps is ALWAYS time-based (`duration_seconds` set, `distance_m: null`), typical 60-180s.
        - Work reps: set EXACTLY ONE of `distance_m` (e.g. 800m rep) OR `duration_seconds` (e.g. 3-min hill).
        - Cooldown is REQUIRED and ALWAYS time-based (`duration_seconds` set, `distance_m: null`), typical 300s (5 min), max 600s.
    ...
    public function schema(JsonSchema $schema): array
    {
        ...
        return [
            ...
            'schedule' => $schema->string()->required()->description('Complete training schedule as JSON: {"weeks":[{"week_number":1,"focus":"base building","days":[{"day_of_week":1,"type":"'.$trainingTypes.'","target_km":5.0,"target_pace_seconds_per_km":null,"description":"Keep conversational pace","target_heart_rate_zone":2,"intervals":[{"kind":"warmup|work|recovery|cooldown","label":"Warm up","distance_m":null,"duration_seconds":60,"target_pace_seconds_per_km":null}]}]}]}. day_of_week: 1=Monday through 7=Sunday. Unscheduled days are rest days — do NOT emit rest-day entries. Most weeks have 3-5 day entries. `intervals` is REQUIRED for `type: interval` days (optional warmup → alternating work/recovery reps → REQUIRED cooldown). Per-interval fields: kind (warmup|work|recovery|cooldown), label (short human name), distance_m (integer meters), duration_seconds (integer seconds), target_pace_seconds_per_km (set on `work` segments with the progressive goal-pace ramp, null on warmup/recovery/cooldown). RULES: warmup is OPTIONAL and ALWAYS time-based (duration_seconds set, distance_m null, max 120s); recovery is ALWAYS time-based (duration_seconds set, distance_m null, typical 60-180s); work reps set EXACTLY ONE of distance_m or duration_seconds; cooldown is REQUIRED at the end and ALWAYS time-based (duration_seconds set, distance_m null, typical 300s, max 600s). Day-level `target_pace_seconds_per_km` is REQUIRED on tempo/threshold/interval days (progress toward goal pace across the plan) and NULL on easy/long_run days. Do NOT set `title` or `total_km` — server computes those.'),
        ];
    }
