...
- **HR-zone auto-derivation** — `POST /profile/heart-rate-zones/derive` (`HeartRateZonesController`) computes the 5-zone table from the runner's own data: median-of-top-5 `max_heartrate` over qualifying recent runs, with Karvonen / Tanaka fallback when run-data is thin (HealthKit `dateOfBirth` + `restingHeartRate`). `users.heart_rate_zones_source` enum tracks origin (`default` / `derived_empirical` / `derived_age` / `manual`). Onboarding has a confirmation step at `/onboarding/zones` between connect-health and overview; the menu HR sheet has a "Recompute" button that re-runs the same derive. Spec: `docs/superpowers/specs/2026-05-08-hr-zones-auto-derive.md`. `RunningProfileService::analyze` no longer touches zones.
