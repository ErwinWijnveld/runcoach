...
3. New workouts auto-sync in the background via a native `HKObserverQuery` (Strava-style) the moment a run lands in Apple Health; the app also pushes on every connect-health screen visit + foreground resume as a fallback
...
Prod API lives at **https://runcoach.laravel.cloud** (see `.laravel-cloud/README.md` for the full monorepo workaround).
...
- **Env vars** to set in Cloud: `APP_KEY`, `APP_URL=https://runcoach.laravel.cloud`, `DB_*`, `APPLE_BUNDLE_ID=com.erwinwijnveld.runcoach`, `ANTHROPIC_API_KEY`, `AI_MODEL=claude-sonnet-4-6`, `AI_PROVIDER=anthropic`, optional `ADMIN_EMAILS`.
...
- **`build-ios.sh`** — runs `flutter build ipa --release --dart-define=API_BASE_URL=https://runcoach.laravel.cloud/api/v1`. Output: `app/build/ios/ipa/RunCoach.ipa`.
...
- Resources:
  - **Users** (`/admin/users`) — full user list (email/name/Pro status/Pro-until/onboarded). Per-row action group grants comp Pro (1 month / 1 year via `EntitlementSyncService::grantComp`) or revokes it (`::expire`). Read-only otherwise (no create — users come from Sign in with Apple). Use this to comp a tester whose RevenueCat purchase can't complete (e.g. Paid Apps Agreement still processing). The action group also has "Send push notification" (per-row) and there's a matching bulk action on selected rows — both send `AdhocPush` (same as `push:send`), with a title+body modal and a no-iOS-device guard. Files: `app/Filament/Resources/Users/`. Tests: `tests/Feature/Filament/UserResourceProActionsTest.php`, `tests/Feature/Filament/UserResourceSendPushTest.php`.
...
- **Completed-run stats on GoalSchedule** — completed day rows show a 44px compliance ring (grade + band color, thresholds mirror Flutter's `ComplianceColors`: good ≥ 8.0 / ok ≥ 5.0) + an actuals line; week headers get `done/total · avg` rollups; the hero shows an SVG compliance ring (84px, app colors `#34C759`/`#E9B638`/`#8F3A3A`, partial `api/resources/views/filament/coach/components/compliance-ring.blade.php`); the edit-day modal renders an app-style Result panel (`day-result-panel.blade.php` fed by `GoalSchedule::resultPanelData`): ring + sub-score bars + Target-vs-Actual table (row rules mirror the app's `_TargetVsActualSection` — no pace row on interval days) + activity extras + AI feedback markdown. Off-plan runs render as blue `_UnplannedRunTile`-style tiles per week via `GoalSchedule::offPlanRunsByWeek` (same query semantics as `attachUnplannedRuns`). All view-layer; compliance is a 1–10 grade everywhere, never a percentage. Spec: `docs/superpowers/specs/2026-06-10-coach-schedule-run-results-design.md`. Tests: `api/tests/Feature/Filament/GoalScheduleResultsTest.php` (note: modal HTML ships via wire:partial, so modal tests inspect the mounted-action schema, not rendered HTML).
...
- **HealthKit background auto-sync** (Strava-style) — native `HKObserverQuery` + `enableBackgroundDelivery(.immediate)` in `ios/Runner/HealthKitBackgroundSync.swift` wakes the app when a new run lands in Apple Health, fetches it via an anchored query, and POSTs to `/wearable/activities` **in Swift** (the Dart engine isn't alive on a background launch) → existing pipeline runs the AI analysis + `WorkoutAnalyzed` push before the user opens the app. No backend changes. Config bridge `nl.runcoach/bg-sync` (Dart `BackgroundSyncService`) hands native the base URL + Sanctum token (wired in `auth_provider`). Foreground sync stays as the fallback. Needs the `healthkit.background-delivery` entitlement (added) + the same capability enabled on the App ID. Spec: `docs/superpowers/specs/2026-06-02-healthkit-background-sync.md`. See `app/CLAUDE.md` → section 20.
...
- **In-app interval editor** — the same "Edit workout" sheet (`edit_day_sheet.dart`) branches on interval days into a block editor (`interval_blueprint_editor.dart`): warm-up, work blocks (reps × distance/time @ pace, recovery) with add/remove, cool-down, and a live "Distance (auto)" footer from the Dart `IntervalBlueprint.estimateTotalKm()` (constant-for-constant mirror of the PHP estimator, pinned by shared test vectors). Saves the grouped blueprint via the same `PATCH /training-days/{day}`; coach-authored standalone rep/rest steps render locked and pass through unchanged. Spec: `docs/superpowers/specs/2026-06-10-app-interval-editor-design.md`.
- **Off-plan runs ("buiten schema")** — auto-match now requires a run on a planned session's EXACT date (`ComplianceScoringService::findMatchingDay`, was ±1 day). Runs on any other day surface as blue `_UnplannedRunTile`s in the weekly schedule + blue `_CellState.unplanned` cells on the dashboard charts (legend "Off-plan"). Tapping a blue tile opens `unplanned_run_sheet.dart` → "Koppel aan training" picks a nearby (±7d) uncompleted session and **relocates that session's calendar entry onto the run's actual date** + scores it, via `POST /wearable/activities/{activity}/link-day` (`TrainingScheduleController::linkActivityToScheduleDay`). Backend attaches `unplanned_runs` per week in `TrainingScheduleController::schedule`. See api/CLAUDE.md → "Off-plan run linking" and app/CLAUDE.md §19.
...
- **Interval-day distance is derived, never stored independently** — `target_km` on interval days always equals `IntervalBlueprint::estimateTotalKm(intervals_json)` (recompute-on-write: optimizer pass + `TrainingDay::saving` hook + one-time backfill migration), so the km shown in the app can never contradict the intervals table beneath it. To change an interval session's distance, change the intervals. See `api/CLAUDE.md` → "Interval-day `target_km` is DERIVED". Spec: `docs/superpowers/specs/2026-06-10-interval-target-km-recompute.md`.
- **Interval compliance is a plausibility model, not avg-vs-zone** — interval days score: max HR must touch the zone below target (Z5 day → peaks ≥ Z4 min, avg HR ignored), whole-run avg pace inside a blueprint band `[work avg, jog pace + 90s]`, distance inside `[work volume, 1.8× target]` (overshoot = a real warmup, only undershoot below the rep volume punishes hard). Replaces the old avg-HR-vs-Z5 + symmetric distance that gave correctly-executed sessions a 2-4. One-off `php artisan compliance:rescore-intervals` rescoring backfill. See `api/CLAUDE.md` → "Interval compliance scoring". Spec: `docs/superpowers/specs/2026-06-10-interval-compliance-scoring-design.md`.
...
- **Shareable run card** — **manual-only** (the old auto-popup on app-open was removed June 2026). Opened via the inline `_ShareThisRunButton` on the training-day detail screen → `RunCelebrationSheet.show` pops a 9:16 share-card sheet (`RunShareCard`) with the route as an animated gold polyline, the verdict (first `**bold**` sentence from `ai_feedback`), and 5 KPIs. Tap "Share" → `RepaintBoundary.toImage()` → PNG → `share_plus` iOS share sheet. **Intro animation**: `RunShareCard` is self-driven by one `AnimationController` (`RunShareCard.introDuration` = 2200ms) — the polyline draws progressively while a "runner" marker (drawn by `RoutePolylinePainter` at the path's leading-edge tangent) rides its tip, and the verdict + KPIs fade/slide in overlapping the tail of the draw so there's no dead gap. The painter is constructed ONCE and repaints off the animation (`super(repaint: progress)`, `progress` is now an `Animation<double>`) so Douglas-Peucker simplification doesn't re-run per frame. The card fires `onIntroComplete` to gate the Share button (so the PNG never captures a half-drawn frame); reduce-motion / `animate: false` jumps to the final state. Routes come from a native `HKWorkoutRouteQuery` Swift bridge (`ios/Runner/WorkoutRoute.swift`, channel `nl.runcoach/workout-route`), persisted in `wearable_activities.raw_data.route` and surfaced via `GET /wearable/activities/{id}/route`. One-time route backfill for the past 30d fires from `workout_sync_lifecycle.dart::_maybeSync` (gated by `route_backfill_done_v1` flag) so older runs become shareable. (Removed with the auto-popup: `celebration_provider.dart`, the `GET /share/celebratable-run` Flutter API method, and the `last_celebrated_activity_id_v1` watermark — the backend `/share/celebratable-run` route is now orphaned.) Spec: `docs/superpowers/specs/2026-05-22-shareable-run-card.md`. Plan: `docs/superpowers/plans/2026-05-22-shareable-run-card.md`.
...
- **Subscriptions (RevenueCat) + Pro gating** — AI endpoints behind `require.pro` (402) + job-level `isPro()` guards; entitlement in `users.pro_active_until`. Fresh-onboarding paywall is the post-plan `PlanPreviewScreen`. **Onboarded-but-not-Pro users** (lapsed sub, admin-revoked, or pre-subscriptions *grandfathered*) are hard-gated to a bare `/paywall` screen on cold start (router gate keyed on `ProEntitlementState.resolved` to avoid flashing payers); run ingestion stays free so history keeps building while locked. Admins comp/revoke Pro via `/admin/users`. See `app/CLAUDE.md` §18 + spec `docs/superpowers/specs/2026-05-19-revenuecat-subscriptions.md`.
- **Workout-detail hero restyle (2026-06-10)** — `TrainingDayHeroCard` is now the asset-less brandkit gradient slab with the spark watermark (option B of `docs/design/2026-06-10-workout-hero-restyle.html`); `finisher.png` no longer used there (still used by goal detail). `TrainingDayActionButtons` became a 2×2 icon+label grid (move/edit/link/to-watch, watch = filled primary) at the END of the scroll content; the top-right ⋯ action sheet stays and opens the same modals.
- **Dashboard recent runs** — `GET /dashboard` returns `recent_runs` (5 newest run-type `wearable_activities` + `training_day_id`/`compliance_score` from their `TrainingResult`; summary shape shared via `WearableActivity::toSummaryPayload()`). Flutter `_RecentRunsSection` on the dashboard (between This-week card and weeks matrix): linked runs show the gold RunBoost spark and open `/schedule/day/{id}`; off-plan runs get the blue plus → `showUnplannedRunSheet` (`compliance_score` ships in the payload but isn't rendered in the list). Spec: `docs/superpowers/specs/2026-06-10-dashboard-recent-runs-design.md`.
